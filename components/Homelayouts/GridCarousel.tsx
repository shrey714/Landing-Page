              ...
              <div className="grid gap-y-6 min-[600px]:gap-y-8 min-[900px]:gap-y-10 items-start grid-rows-[auto] grid-cols-[minmax(0,1fr)]">
                ...
                <div className="w-full">
                  <div className="flex flex-col gap-y-6 items-start w-full mt-8">
                    ...
                    <div
                      className="relative w-[calc(100%-2rem)] mx-auto rounded-2xl border border-white/10 h-115 md:h-125 lg:h-150 overflow-hidden bg-white/6"
                      style={{
                        boxShadow:
                  ...
                  <div className="flex relative min-w-0 col-span-full mt-10">
                    <ul className="relative flex items-center justify-center list-none m-0 p-0 min-w-full min-[900px]:grid min-[900px]:grid-flow-col min-[900px]:auto-cols-fr min-[900px]:items-stretch">
                      ...
                      <div className="absolute -top-5 left-0 w-full h-1 rounded-full bg-white/20 max-[899px]:hidden pointer-events-none" />

                      {slides.map((slide, index) => {
                        const nextColor = slides[index + 1] ? slides[index + 1].color : slide.color;
                        const bgGradient = `linear-gradient(90deg, ${slide.color}, ${nextColor})`;
                        ...
                        >
                          ...
                          <motion.div
                            ...
                            className={`absolute -top-5 left-0 h-1 max-[899px]:hidden origin-left ${index === 0 ? "rounded-l-full" : ""} ${index === slides.length - 1 ? "rounded-r-full" : ""}`}
                            initial={{ width: index < activeSlide ? "100%" : "0%", background: bgGradient }}
                            animate={{ width: index <= activeSlide ? "100%" : "0%", background: bgGradient }}
                            transition={{ duration: activeSlide === index ? 4 : 0, ease: "linear" }}
                          />

                          <button
                            className={`cursor-pointer outline-none border-none bg-transparent py-2 px-3 text-sm transition-opacity duration-300 max-[899px]:hidden ${activeSlide === index
                                ? "opacity-100"
